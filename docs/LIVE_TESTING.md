...
| Private-message PATCH returns 200 but creates a replacement instead of updating the requested ID | The original must remain unchanged and the collection must contain a different ID with the exact submitted fields. With cleanup enabled, the runner deletes the replacement and proves it absent; `cleanup: never` retains and reports it without issuing a delete. | [Message form without `message_id`](https://github.com/joomla/joomla-cms/blob/6.1.2/administrator/components/com_messages/forms/message.xml) |
...
Private-message create fixtures deliberately omit `state`. Joomla's recipient
`MessageModel::getItem()` marks an existing message read as part of the item
read, so cross-path verification can validly observe the state transition
from unread (`0`) to read (`1`). Subject, recipient, priority, body, exact ID,
and collection visibility remain strict postconditions.
